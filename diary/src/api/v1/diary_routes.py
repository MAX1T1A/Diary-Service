...
@router.get("/diary")
def get_list_diaries(author: User = Depends(JWTBearer()), diary_service: DiaryServices = Depends(stub_diary_service)) -> List[DiaryGet]:
    return [DiaryGet(**diary.to_dict()) for diary in diary_service.find_many(user_id=author)]


@router.post("/diary")
def add_diary(request: DiaryUniversal, author: User = Depends(JWTBearer()), diary_service: DiaryServices = Depends(stub_diary_service)) -> int:
    diary_service.create(name=request.name, user_id=author)
    return status.HTTP_201_CREATED


@router.put("/diary/{diary_id}")
def update_diary(diary_id: int, request: DiaryUniversal, author: User = Depends(JWTBearer()), diary_service: DiaryServices = Depends(stub_diary_service)) -> int:
...
def delete_diary(diary_id: int, author: User = Depends(JWTBearer()), diary_service: DiaryServices = Depends(stub_diary_service)) -> int:
    diary = diary_service.find_one(id=diary_id, user_id=author)
    if not diary:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="This diary doesn't exist.")
