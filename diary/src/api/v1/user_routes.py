...
from models.schemas import Login, UserInSchemas
...
def user_register(
    request: UserInSchemas,
    user_service: UserServices = Depends(stub_user_service),
    hasher: Hash = Depends(get_hasher),
) -> int:
    ...
    user_service.create(
        name=request.name, email=request.email, password=hasher.bcrypt(request.password)
    )
...
def login(
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="This user is not registered."
        )
